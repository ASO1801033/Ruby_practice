...
file = File.open("pra_30_file.txt", "w") #pra_30_file.txt：ファイル名 / "w"：書き込みを行う
file.puts("書き込みされたよ！(write)") #新しく書き込むデータ
file.close #ファイルを閉じる

file = File.open("pra_30_file.txt", "r") #pra_30_file.txt：ファイル名 / "r"：読み込みを行う
puts "write実行後のpra_30_file.txtの中身"
puts file.read #ファイルの中身を読み込んで表示
file.close #ファイルを閉じる

puts ""

file = File.open("pra_30_file.txt", "a") #pra_30_file.txt：ファイル名 / "a"：末尾に追記を行う
file.puts("追記されたよ！(append)") #追記するデータ
file.close #ファイルを閉じる

file = File.open("pra_30_file.txt", "r") #pra_30_file.txt：ファイル名 / "r"：読み込みを行う
puts "append実行後のpra_30_file.txtの中身"
puts file.read #ファイルの中身を読み込んで表示
file.close #ファイルを閉じる
